...
旧版の主要な画面機能はおおむねPWA版へ反映済み。残る差分は、PWAでは不要なPCサーバー運用機能と、任意の通知機能が中心。
...
| 図鑑詳細 | 新しいものから最大12枚の関連写真を表示 | 対応済み | 成長・季節変化を見返せる | 低 |
| 図鑑詳細 | 観察履歴を表示する | 対応済み | 図鑑から観察へ戻れる | 低 |
...
| 診断 | Gemini CLI、DB、画像フォルダ、ログ、Tailscaleを診断 | PWA向け診断に置換済み。APIキー、モデル、端末保存容量、画像容量、バックアップ対象件数を表示 | PWAに必要な状態確認はできる | 低 |
...
| ログ | `data/logs/server.log` に解析・通知・エラーを記録 | 対応済み。IndexedDBに解析・図鑑生成・バックアップの履歴を保存し、設定画面で確認/削除できる | 実機トラブル時の原因追跡に使える | 低 |
| Discord通知 | 解析完了・失敗をWebhook通知 | 対応不要。PWA版では端末単体利用を優先し、Discord通知は実装対象外 | 通知運用をする場合は別途検討 | 対応不要 |
...
対応済み。旧版の「同じ植物の成長や季節変化を見返す」価値に近づけた。
...
旧版に近づけるための優先実装は一通り完了。以後は実機確認で見つかった不具合修正、または利用上必要になった任意機能を個別に追加する。
