...
| 観察詳細 | 候補から植物名・学名を修正欄へ反映できる | 候補選択UIなし | 低信頼解析の復旧が手間 | 高 |
...
| 観察詳細 | 解析中の強制停止ボタンがある | `cancelRequested` 型はあるが実処理・UI未実装 | 長時間実行を止められない | 中 |
| 解析堅牢性 | スキーマ違反時に再試行し、別スキーマも正規化する | ある程度の正規化はあるが、旧版ほど網羅していない | 軽量モデル利用時に失敗しやすい | 高 |
| 解析堅牢性 | Gemini CLIの空応答・非JSON応答に再試行する | API応答のJSON抽出はあるが、プロンプト再試行は限定的 | 図鑑項目欠落や解析失敗が残りやすい | 高 |
| 図鑑生成 | 図鑑項目が欠けた場合に補完生成する | 欠落補完ループなし | 手入れメモなどが空のまま残る可能性 | 高 |
...
旧版で実際に問題になった領域。軽量モデル対応を重視するなら、PWA版にも移植するべき。
...
1. `候補クリックで手動修正フォームへ反映`
2. `Gemini応答の再試行・正規化強化`
3. `図鑑詳細の観察履歴と複数写真表示`
4. `旧版データ移行スクリプト`
5. `保存容量・APIキー・バックアップの診断表示`
6. `解析中の強制停止`
7. `Discord通知など任意機能`
